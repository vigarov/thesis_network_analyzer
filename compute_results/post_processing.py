...
`K_SAMPLES` post-nonlinearity activations are exactly zero.
...
- Runs that include at least one pretraining trial (some trial name contains
  ...
  not that trial's last checkpoint never set `is_dead`.
  ...
  on all digits 
	--> dead is only reported from the end of pretraining
  onward 
