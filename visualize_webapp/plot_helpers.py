...
from visualize_webapp.constants import _NETWORK_N_DIGITS
...
def add_trial_boundaries_mpl(
    ...
    *,
    ...
    `d` stepping as in `experiments.mnist.cat2.sequence_recover` /
    `sequence_reinforce` (anchor `(digitA + 2*r) % 10` per run `r`). Required
    for those trials; raises `ValueError` if they appear and `first_digit` is
    `None`.
