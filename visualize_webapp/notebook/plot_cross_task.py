...
from visualize_webapp.common import _compact_neuron_label, _is_dead_column_to_bool
...
def concat_reactivated_summaries(
    ...
    """Same row MultiIndex convention as `concat_reassigned_summaries`."""
    pieces: list[pd.DataFrame] = []
    for experiment_id, optimizer_id, df in rows:
...
def optimizer_ids_for_experiment_runs(
    ...
    `load_summaries_for_experiment_runs` because empty summaries add no rows.
    """
    seen: dict[str, None] = {}
...
def load_summaries_for_experiment_runs(
    ...
    `optimizer_ids_for_experiment_runs` for the full list from discovery.
    """
